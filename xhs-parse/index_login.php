...
// --- Server-side login + shared cookie -------------------------------------
// A single shared password gates the admin functions (saving/clearing the
// cookie). The cookie itself is stored in a plain file next to this script
// (.xhs_cookie, mode 0600). Any device that logs in reads the same file, so
// the same cookie is reused everywhere; a newly pasted cookie overwrites it.
const ADMIN_PASSWORD = 'mms4787614';
const COOKIE_FILE_NAME = '.xhs_cookie';
const SESSION_COOKIE_NAME = 'xhs_admin';

function cookieFilePath()
{
    return __DIR__ . '/' . COOKIE_FILE_NAME;
}

function loadServerCookie()
{
    $path = cookieFilePath();
    if (!is_file($path)) return '';
    $raw = @file_get_contents($path);
    $value = is_string($raw) ? trim($raw) : '';
    // The file is plain-text cookie on the first line; ignore the rest.
    if ($value === '') return '';
    $nl = strpos($value, "\n");
    return $nl === false ? $value : trim(substr($value, 0, $nl));
}

function saveServerCookie($value)
{
    $path = cookieFilePath();
...
function clearServerCookie()
{
    ...
    return @unlink($path) || @file_put_contents($path, "", LOCK_EX);
}

/**
 * Resolve the admin session flag from the request. The flag is set by the
 * admin session cookie; /api/login compares the password and sets it.
 */
function isAdminLoggedIn()
{
    $cookies = isset($_COOKIE) && is_array($_COOKIE) ? $_COOKIE : [];
    return isset($cookies[SESSION_COOKIE_NAME]) &&
        is_string($cookies[SESSION_COOKIE_NAME]) &&
        $cookies[SESSION_COOKIE_NAME] === '1';
}

function setAdminSession($loggedIn)
{
    if ($loggedIn) {
        // HttpOnly + SameSite=Lax, 7 days. The cookie value is a fixed flag;
        // the password itself never travels after login.
...
/**
 * Resolve the cookie to use for a parse: the client-supplied cookie wins; when
 * the client sends none (or an empty string) and a server-side cookie is
 * stored, that stored value is used. This is what makes "any logged-in device
 * picks up the shared cookie".
 */
function resolveCookie($clientCookie)
{
    if (is_string($clientCookie) && trim($clientCookie) !== '') return $clientCookie;
    return loadServerCookie();
}
...
#adminbox code{font-family:var(--mono);font-size:12px;background:#10131a;padding:1px 5px;
border-radius:4px;color:var(--acc)}
#adminbox .cookrow input[type=password]{background:#10131a;color:var(--tx);border:1px solid var(--line);
border-radius:8px;padding:8px 10px;font:inherit;font-family:var(--mono);font-size:12px}
#adminbox .cookrow input[type=password]:focus{outline:none;border-color:var(--acc)}
#loginmsg{color:var(--err);font-size:12px;margin-top:6px;display:block}
...
.lb .nav:hover{opacity:1}.lb .prev{left:6px}.lb .next{right:6px}
 .err{border-color:#5b2a26;background:#1c1416}
/* 处理步骤 / 错误步骤：长链接一律折行并装在框内（再长也滚动，不撑破右边） */
.steps{color:var(--dim);font-size:12px;white-space:pre-wrap;font-family:var(--mono);
overflow-wrap:anywhere;word-break:break-word;max-height:280px;overflow:auto;
padding:8px;border:1px solid var(--line);border-radius:8px;background:#0e1117;margin-top:6px}
/* 右上角汉堡按钮 + 下拉登录面板 */
#burger{position:fixed;top:12px;right:14px;z-index:60;width:40px;height:40px;border:1px solid var(--line);
border-radius:10px;background:#171a21;color:var(--tx);cursor:pointer;display:flex;flex-direction:column;
justify-content:center;align-items:center;gap:4px;box-shadow:0 2px 8px rgba(0,0,0,.35)}
#burger:hover{border-color:var(--acc)}
#burger span{display:block;width:18px;height:2px;background:var(--tx);border-radius:2px}
#loginmsg{color:var(--err);font-size:12px;margin-top:6px;display:block}
#adminbox{position:fixed;top:58px;right:14px;z-index:59;width:320px;max-width:calc(100vw - 28px);
max-height:calc(100vh - 74px);overflow:auto;background:#171a21;border:1px solid var(--line);
border-radius:12px;padding:14px;box-shadow:0 8px 30px rgba(0,0,0,.5);
display:none}
#adminbox code{font-family:var(--mono);font-size:12px;background:#10131a;padding:1px 5px;
border-radius:4px;color:var(--acc)}
#adminbox .cookrow input[type=password]{background:#10131a;color:var(--tx);border:1px solid var(--line);
...
<!-- 右上角汉堡：点击展开登录 / 服务器共享 Cookie 管理 -->
<button id="burger" title="登录 / 服务器共享 Cookie" type="button">
<span></span><span></span><span></span>
</button>
<div id="adminbox">
<h3 class="sec" style="margin:0 0 8px">登录 · 服务器共享 Cookie</h3>
<div class="cookrow" style="margin-top:0;margin-bottom:8px">
<button class="sm primary" id="loginsave">保存本机 Cookie 到服务器</button>
<button class="sm" id="logout" style="display:none">退出登录</button>
<span class="muted" id="adminstate">未登录</span>
</div>
<div id="loginform">
<div class="cookrow" style="margin-top:0">
<input id="password" type="password" autocomplete="off" spellcheck="false"
placeholder="管理员密码" style="flex:1;min-width:0">
<button class="sm primary" id="loginbtn" type="button">确认登录</button>
</div>
<span id="loginmsg"></span>
</div>
<div id="cookadmin" style="margin-top:12px">
<p class="muted" style="margin:0 0 8px">登录后可修改服务器共享 Cookie（保存于 <code>.xhs_cookie</code>，所有设备通用）。填入新的会覆盖旧的；留空并保存即清除。</p>
<div class="cookrow" style="margin-top:0">
<span class="muted" id="sharedstate">服务器未保存 Cookie</span>
<button type="button" class="sm" id="sharedclear">清除共享</button>
</div>
<p class="muted" style="margin:8px 0 0">任何设备登录后读取同一文件；未登录时不读取。</p>
</div>
</div>
<div id="result"></div>
...
// ---------------------------------------------------------------------------
// 管理：登录 + 服务器共享 Cookie
// 登录后可保存/查看服务器上的 .xhs_cookie；任何设备登录后读取同一文件。
// 未登录时不会读取 cookie。填入新值会覆盖旧值。
// ---------------------------------------------------------------------------
var adminBoxEl=document.getElementById("adminbox"),
loginBtn=document.getElementById("burger"),
loginFormEl=document.getElementById("loginform"),
loginBtnDo=document.getElementById("loginbtn"),
logoutBtn=document.getElementById("logout"),
adminStateEl=document.getElementById("adminstate"),
loginMsgEl=document.getElementById("loginmsg"),
cookAdminEl=document.getElementById("cookadmin"),
sharedStateEl=document.getElementById("sharedstate"),
sharedClearBtn=document.getElementById("sharedclear"),
passwordEl=document.getElementById("password");
function refreshAdminUI(){
fetch(api("/api/cookie/status")).then(function(r){return r.json();}).then(function(j){
if(!j.ok)return;
var logged=j.loggedIn;
loginFormEl.style.display=logged?"none":"block";
logoutBtn.style.display=logged?"":"none";
cookAdminEl.style.display=logged?"":"none";
adminStateEl.textContent=logged?("已登录 · 共享Cookie"+(j.hasCookie?("（"+j.cookieLength+" 字符）"):"（未保存）")):"未登录";
sharedStateEl.textContent=j.hasCookie?("服务器已保存 Cookie（"+j.cookieLength+" 字符，前缀 "+j.cookiePrefix+"…）"):"服务器未保存 Cookie";
// 登录后自动填入服务器 cookie 到本机输入框（若本机为空）
if(logged&&j.hasCookie){
if(!cookieEl.value.trim()){cookieEl.value="";saveCookiePref();}
}
if(!logged){loginMsgEl.textContent="";}
}).catch(function(){});
}
// 右上角汉堡按钮：点击展开/收起右上角登录面板
loginBtn.addEventListener("click",function(){
var open=adminBoxEl.style.display==="";
if(open){adminBoxEl.style.display="none";}
else{refreshAdminUI();adminBoxEl.style.display="block";}
});
// 点击面板外区域自动收起
document.addEventListener("click",function(e){
if(adminBoxEl.style.display==="block" && !adminBoxEl.contains(e.target) && e.target!==loginBtn && !loginBtn.contains(e.target)){
adminBoxEl.style.display="none";
}
});
loginBtnDo.addEventListener("click",async function(){
var pw=passwordEl.value;
loginMsgEl.textContent="";
if(!pw){loginMsgEl.textContent="请输入密码";return;}
loginBtnDo.disabled=true;
try{
...
cookAdminEl.style.display="";
adminStateEl.textContent="已登录";
refreshAdminUI();
}
else{loginMsgEl.textContent=j.error&&j.error.message?j.error.message:"登录失败";}
}catch(e){loginMsgEl.textContent="网络错误："+e;}
finally{loginBtnDo.disabled=false;}});
logoutBtn.addEventListener("click",async function(){
try{var r=await fetch(api("/api/logout"),{method:"POST"});var j=await r.json();}catch(e){}
loginFormEl.style.display="block";
logoutBtn.style.display="none";
cookAdminEl.style.display="none";
adminStateEl.textContent="未登录";
loginMsgEl.textContent="";
passwordEl.value="";
});
// 保存共享 Cookie：把输入框里的值 POST 到服务器（覆盖旧的）
document.getElementById("loginsave").addEventListener("click",async function(){
var v=getCookie();
if(!v){setStatus("Cookie 输入框为空，先填入再保存");return;}
try{
var r=await fetch(api("/api/cookie/save"),{method:"POST",headers:{"Content-Type":"application/json"},
body:JSON.stringify({cookie:v})});
var j=await r.json();
if(r.ok&&j.ok){setStatus("共享 Cookie 已保存（"+j.length+" 字符）");refreshAdminUI();}
else{onErrText(j.error&&j.error.message?j.error.message:"保存失败（未登录？）");}
}catch(e){onErrText("网络错误："+e);}});
sharedClearBtn.addEventListener("click",async function(){
try{
var r=await fetch(api("/api/cookie/save"),{method:"POST",headers:{"Content-Type":"application/json"},
body:JSON.stringify({cookie:""})});
var j=await r.json();
if(r.ok&&j.ok){setStatus("共享 Cookie 已清除");refreshAdminUI();}
else{onErrText("清除失败");}
}catch(e){onErrText("网络错误："+e);}});
// 页面加载时刷新管理状态（若已登录会自动填入共享 cookie）
refreshAdminUI();
...
function handleParse()
{
    ...
    $options = [
        ...
        // Client cookie wins; otherwise fall back to the shared server-side cookie.
        'cookie' => resolveCookie($source['cookie'] ?? getParam('cookie')),
...
{
    try {
        $result = fetchSampleLink(resolveCookie(getParam('cookie')));
    } catch (XhsParseError $error) {
        jsonResponse(
            statusForCode($error->errorCode),
            errorBody($error->errorCode, $error->getMessage(), $error->detail)
...
// ---------------------------------------------------------------------------
// Admin: login / logout / cookie save / cookie status / cookie clear
//
// These endpoints back the "登录" button on the built-in page. A single shared
// password (ADMIN_PASSWORD) gates them; the cookie itself lives in a plain
// file next to this script, so every logged-in device reads the same value.
// ---------------------------------------------------------------------------

function requireAdmin()
{
    if (!isAdminLoggedIn()) {
        jsonResponse(401, errorBody('not_logged_in', '请先登录'));
    }
}
...
function handleLogin()
{
    if (requestMethod() !== 'POST') {
        jsonResponse(405, errorBody('bad_method', '登录仅支持 POST'));
    }
    $raw = @file_get_contents('php://input');
    ...
    if ($password !== '' && $password === ADMIN_PASSWORD) {
        ...
        jsonResponse(200, ['ok' => true, 'message' => '登录成功']);
    }
    jsonResponse(401, errorBody('bad_password', '密码错误'));
}

function handleLogout()
{
    setAdminSession(false);
    jsonResponse(200, ['ok' => true, 'message' => '已退出登录']);
}

/** Save (or overwrite) the shared cookie. Admin only. */
function handleSaveCookie()
{
    requireAdmin();
    if (requestMethod() !== 'POST') {
        jsonResponse(405, errorBody('bad_method', '保存 Cookie 仅支持 POST'));
    }
    $raw = @file_get_contents('php://input');
    $payload = is_string($raw) ? json_decode($raw, true) : null;
    $value = (is_array($payload) && isset($payload['cookie']) && is_string($payload['cookie']))
        ? trim($payload['cookie'])
        : '';
    // An empty cookie means "clear the shared cookie".
    if ($value === '') {
        clearServerCookie();
        jsonResponse(200, ['ok' => true, 'cleared' => true, 'length' => 0]);
    }
    if (!saveServerCookie($value)) {
        jsonResponse(500, errorBody('save_failed', '写入服务器 Cookie 文件失败（目录不可写？）'));
    }
    // Re-read to report the stored length (trusts the file, not the input).
    $stored = loadServerCookie();
    jsonResponse(200, [
        'ok' => true,
        'cleared' => false,
...
/** Status of the shared cookie + admin session. Read-only; not gated. */
function handleCookieStatus()
{
    ...
    jsonResponse(200, [
        ...
        'loggedIn' => $loggedIn,
        'hasCookie' => $stored !== '',
        'cookieLength' => strlen($stored),
        'cookiePrefix' => $stored !== '' ? substr($stored, 0, 24) : '',
    ]);
}
...
const HELP = [
    ...
    'endpoints' => [
        ...
        'POST /api/login' => '管理员登录（密码认证），成功后设置会话 Cookie',
        'POST /api/logout' => '退出管理员会话',
        'POST /api/cookie/save' => '保存/覆盖服务器共享 Cookie（需管理员）；传空则清除',
        'GET /api/cookie/status' => '查询共享 Cookie 是否存在 + 当前是否已登录（只读）',
    ],
    'options' => [
...
function dispatch()
{
    ...
    try {
        ...
        if ($path === '/api/login' || $path === '/login') return handleLogin();
        if ($path === '/api/logout' || $path === '/logout') return handleLogout();
        if ($path === '/api/cookie/save') return handleSaveCookie();
        if ($path === '/api/cookie/status') return handleCookieStatus();
