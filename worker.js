...
/** Append one diagnostic line, skipping an immediate repeat (retry loops). */
function noteStep(steps, message) {
  if (!steps) return;
  if (steps[steps.length - 1] === message) return;
  steps.push(message);
}

/** DouyinInspector.performNativeFetch primary source: /aweme/v1/web/aweme/detail/. */
async function fetchDetailApi(jar, awemeId, steps) {
  ...
  if (status !== 200) {
    // A 403 here is not "the item has no data": douyin's edge refuses the
    // request outright. Cloudflare's workerd (what `wrangler dev` runs) cannot
    // present a browser-like TLS fingerprint, so the SEO-crawler allowance that
    // works from other HTTP clients never applies. Say so, instead of letting
    // the caller report the misleading "no clean assets".
    noteStep(steps, `详情接口返回 HTTP ${status}`
      + (status === 403
        ? '：抖音风控拒绝当前运行时（wrangler/workerd 的客户端指纹被判定为机器人，换 UA/Cookie 均无效）'
        : ''));
    return null;
  }
  if (!isJsonText(body)) {
    noteStep(steps, `详情接口返回非 JSON 响应（${body ? body.length + ' 字节' : '空 body'}），疑似被风控拦截`);
    return null;
  }
  let root;
  try {
...
async function fetchDetailFromPages(jar, awemeId, steps) {
  ...
  for (const target of pageTargets) {
    try {
      ...
      if (status !== 200 || !body) {
        noteStep(steps, `页面探测 ${target.url} → HTTP ${status}（无内容）`);
        continue;
      }
      const detail = extractDetailFromHtml(body, awemeId);
      if (detail) {
        noteStep(steps, `页面探测 ${target.url} → HTTP 200，已取得作品数据`);
        return { detail, source: 'page_embed', finalUrl };
      }
      noteStep(steps, `页面探测 ${target.url} → HTTP 200，但页面不含作品数据（${body.length} 字节；详情接口当前不可用时该页面通常只提供带水印的 playwm 源）`);
    } catch (err) {
      if (err instanceof ParseError) throw err;
      // try the next page on transient network errors
      noteStep(steps, `页面探测 ${target.url} 失败：${err && err.message ? err.message : err}`);
    }
  }
  return null;
}
...
/** Douyin's hosted BGM files (`…/ies-music/<id>.mp3`). */
function isHostedMusicUrl(url) {
  return !!url && /\.mp3(\?|$)/i.test(url) && /music/i.test(url);
}

/**
 * Clean mp3 BGM mirrors from aweme_detail.music, with a fallback for the
 * page-only shape (see below). Used for gallery works.
 */
function collectMusicMirrors(awemeDetail) {
  ...
  if (out.length === 0) {
    // Public share pages — the only source once douyin's detail API is refused —
    // ship `music` with no play address at all, yet still wrap the BGM mp3 as the
    // post's pseudo `video`: `video.play_addr.uri` *is* the `ies-music/*.mp3` URL,
    // and its `url_list` entry carries the same URL as the `video_id` of a
    // playwm wrapper. Recover it so the background music is not lost.
    const video = asObj(awemeDetail.video);
    const addr = video ? (asObj(video.play_addr) || asObj(video.playAddr)) : null;
    if (addr) {
      const uri = strTrim(addr.uri);
      if (isHostedMusicUrl(uri)) add(uri);
      for (const u of extractUrls(addr)) {
        const embedded = /[?&]video_id=([^&]+)/i.exec(u);
        if (!embedded) continue;
        let candidate = null;
        try {
          candidate = decodeURIComponent(embedded[1]);
        } catch (_) {
          candidate = embedded[1];
        }
        if (isHostedMusicUrl(candidate)) add(candidate);
      }
    }
  }
  return out;
...
/** Raw video ids the mobile play endpoint accepts as `video_id` (v0…). */
const VIDEO_ID_RE = /^v0[0-9a-z]{8,}$/i;

/** Rendition the derived clean address is pinned to. */
const PINNED_RATIO = '1080p';

/**
 * Douyin publishes the same rendition twice: `/aweme/v1/playwm/` (watermarked)
 * and `/aweme/v1/play/` (clean). The page-hydration fallback only ever carries
 * the `playwm` mirror (the detail API that would list clean ones is refused by
 * douyin's WAF under the Workers runtime), so switching the endpoint segment
 * back yields the clean address for the very same video_id. Returns null when
 * the URL is not a `playwm` address.
 */
function deriveCleanPlayUrl(url) {
  if (!url || !/\/playwm(?=[/?]|$)/i.test(url)) return null;
  let derived = url.replace(/\/playwm(?=[/?]|$)/i, '/play');
  if (/[?&]watermark=/i.test(derived)) {
    // `watermark=1` was addressed to the watermarked endpoint; drop it so the
    // derived address is not immediately re-flagged by isWatermarkedMediaUrl().
    derived = derived
      .replace(/([?&])watermark=[^&]*/gi, '$1')
      .replace(/([?&])&+/g, '$1')   // "?&&" -> "?" and "&&" -> "&"
      .replace(/[?&]+$/, '');       // drop a trailing separator
  }
  // The share page signs its watermarked address for `ratio=720p`; the clean
  // endpoint serves the same video_id at 1080p, so pin the rendition to 1080p
  // instead of inheriting whatever the page happened to use.
  if (/[?&]ratio=/i.test(derived)) {
    derived = derived.replace(/([?&])ratio=[^&]*/gi, (_m, sep) => `${sep}ratio=${PINNED_RATIO}`);
  } else {
    derived += (derived.includes('?') ? '&' : '?') + `ratio=${PINNED_RATIO}`;
  }
  return derived;
}

/** Clean mirrors derived from a `playwm`-only candidate; empty when none exist. */
function deriveCleanPlayMirrors(candidate) {
  if (!candidate) return [];
  const derived = new Set();
  for (const url of candidate.urls) {
    const clean = deriveCleanPlayUrl(url);
    if (clean && !isDeadMediaLink(clean) && !isWatermarkedMediaUrl(clean)) derived.add(clean);
  }
  // Page-hydration payloads occasionally carry only `play_addr.uri` (the raw
  // video id) with no url_list at all.
  if (derived.size === 0 && candidate.uri && VIDEO_ID_RE.test(candidate.uri)) {
    derived.add('https://aweme.snssdk.com/aweme/v1/play/?video_id='
      + encodeURIComponent(candidate.uri) + `&ratio=${PINNED_RATIO}&line=0`);
  }
  return [...derived];
}

/**
 * First video candidate (quality order) that has any clean URL; its clean
 * mirrors. A candidate that only offers `playwm` is the last resort — its
 * mirrors are derived, and `state.derived` records that so the caller can say so
 * instead of reporting "no clean assets".
 */
function bestCleanVideoMirrors(candidates, state) {
  for (const candidate of candidates) {
    const clean = cleanUrls(candidate.urls);
    if (clean.length) return clean;
  }
  for (const candidate of candidates) {
    const derived = deriveCleanPlayMirrors(candidate);
    if (derived.length) {
      if (state) state.derived = true;
      return derived;
    }
  }
  return [];
...
function bestCleanVideoCandidate(candidates) {
  ...
  for (const candidate of candidates) {
    if (deriveCleanPlayMirrors(candidate).length) return candidate;
  }
  return null;
}
...
function buildPreview(plan) {
  ...
  let assetCount = 0;
  let liveItemCount = 0;
  const mirrorState = { derived: false };

  if (!plan.isGallery) {
    const videoMirrors = bestCleanVideoMirrors(plan.videoCandidates, mirrorState);
    ...
    return {
      ...
      assetCount,
      liveItemCount: 0,
      derivedCleanMirrors: mirrorState.derived,
      items,
    };
  }
  ...
  for (const galleryItem of plan.galleryItems) {
    ...
    const motionMirrors = bestCleanVideoMirrors(galleryItem.liveCandidates, mirrorState);
  ...
  // The BGM belongs to the gallery post itself. It used to be attached only when
  // a live clip was also found, which silently hid the music whenever the motion
  // video was unavailable (e.g. page-only data under the Workers runtime).
  if (plan.audioMirrors && plan.audioMirrors.length) {
...
async function doParse(text, steps, jar) {
  ...
  //    rides through douyin's transient per-IP 403 windows.
  //    NOTE: under Cloudflare's workerd (what `wrangler dev` runs) this endpoint
  //    answers 403 for *every* UA/Cookie combination, because the runtime's TLS
  //    fingerprint is classified as a bot; the page-probe fallback below (plus
  //    the playwm -> play derivation) is then the only working path.
  steps.push('正在建立会话并读取详情接口 JSON');
  let result = null;
  try {
    for (let attempt = 0; attempt < 3 && !result; attempt++) {
      await primeSession(jar, awemeId);
      result = await fetchDetailApi(jar, awemeId, steps);
      if (!result && attempt < 2) {
  ...
  if (!result) {
    ...
    result = await fetchDetailFromPages(jar, awemeId, steps);
  }
  if (!result) {
  ...
  if (preview.derivedCleanMirrors) {
    steps.push('页面数据只给出带水印的 playwm 地址，已派生同一 video_id 的无水印 play 地址');
  }
  if (preview.assetCount <= 0) {
    throw new ParseError(
      'no_clean_assets',
      source === 'page_embed'
        ? '作品页面只提供了带水印的 playwm 源，且未能派生出无水印地址（详情接口在当前运行时被风控拒绝，详见步骤）'
  ...
  const output = {
    ...
    derived_clean_mirrors: !!preview.derivedCleanMirrors,
    items: preview.items,
    steps,
  };
...
const HELP = {
  ...
  response: {
    ...
    asset_count: '素材文件数',
    'derived_clean_mirrors': '镜像是否由带水印的 playwm 地址派生而来（详情接口被风控拒绝时，页面数据只提供 playwm）',
    items: '每项含 assets[].mirrors（可直接下载的干净镜像列表，按优先级排序）',
  },
  notes: [
    '仅用于你有权处理的内容；平台页面结构、验证状态或地区差异都可能影响结果。',
    '带签名的媒体地址只返回给调用方，不会写入日志。',
    '抖音详情接口受风控保护：Cloudflare Workers / workerd（wrangler dev 的运行时）的客户端指纹会被判定为机器人并返回 403，换 UA/Cookie 均无效。此时解析改用作品页面数据：视频类会把页面唯一的带水印 playwm 地址派生为同一 video_id 的无水印 play 地址（固定 ratio=1080p，响应中的 derived_clean_mirrors 为 true）；图集/实况类会从页面数据里回收 BGM mp3，但实况的动态视频只存在于被拒的详情接口中，因此该环境下只能给出静态图与背景音乐。详情接口可用时镜像一律原样输出、不做任何改写。',
  ],
};
...
function selftestPlaywmDerivation() {
  // The page-hydration fallback only ever exposes the watermarked playwm
  // address; the derived play address must be used instead of failing.
  const playwmOnly = {
    aweme_id: 'video-9',
    desc: 'pw',
    video: {
      play_addr: {
        uri: 'v0d00fg10000dakq09vog65o77e0k78g',
        url_list: [
          'https://aweme.snssdk.com/aweme/v1/playwm/?video_id=v0d00fg10000dakq09vog65o77e0k78g&ratio=720p&line=0',
        ],
      },
    },
  };
  const derivedPreview = buildPreview(selectMedia(playwmOnly));
  const expected = 'https://aweme.snssdk.com/aweme/v1/play/'
    + '?video_id=v0d00fg10000dakq09vog65o77e0k78g&ratio=1080p&line=0';
  if (derivedPreview.assetCount !== 1) throw new Error('playwm derivation: assetCount');
  if (derivedPreview.items[0].assets[0].mirrors[0] !== expected) {
    throw new Error('playwm derivation: mirror ' + derivedPreview.items[0].assets[0].mirrors[0]);
  }
  if (!derivedPreview.derivedCleanMirrors) throw new Error('playwm derivation: flag');
  if (isWatermarkedMediaUrl(expected)) throw new Error('playwm derivation: still watermarked');

  // A candidate that already has a clean URL must never be replaced by one.
  const withClean = {
    aweme_id: 'video-10',
    desc: 'clean',
    video: {
  ...
  const cleanPreview = buildPreview(selectMedia(withClean));
  if (cleanPreview.derivedCleanMirrors) throw new Error('playwm derivation: unexpected derivation');
  if (cleanPreview.items[0].assets[0].mirrors[0] !== 'https://cdn.example/clean.mp4') {
    throw new Error('playwm derivation: clean order');
  }

  // `playwm` with an explicit watermark query loses that query on derivation,
  // and the rendition is pinned to 1080p.
  const watermarkQuery = deriveCleanPlayUrl(
    'https://api.amemv.example/aweme/v1/playwm/?video_id=v0123456789&watermark=1&line=0',
  );
  if (watermarkQuery !== 'https://api.amemv.example/aweme/v1/play/?video_id=v0123456789&line=0&ratio=1080p') {
    throw new Error('playwm derivation: watermark query ' + watermarkQuery);
  }
  if (deriveCleanPlayUrl('https://api.amemv.example/aweme/v1/playwm/?video_id=v0123456789') !== 'https://api.amemv.example/aweme/v1/play/?video_id=v0123456789&ratio=1080p') {
    throw new Error('playwm derivation: ratio appended to bare url');
  }
  if (deriveCleanPlayUrl('https://cdn.example/play/a.mp4') !== null) {
    throw new Error('playwm derivation: non-playwm rewritten');
  }

  // Page payloads that carry only play_addr.uri (no url_list) still resolve.
  const uriOnly = buildPreview(selectMedia({
    aweme_id: 'video-11',
    desc: 'uri-only',
    video: { play_addr: { uri: 'v0d00fg10000dakq09vog65o77e0k78g' } },
  }));
  if (uriOnly.assetCount !== 1 || !uriOnly.derivedCleanMirrors) {
    throw new Error('playwm derivation: uri-only');
  }
  if (!uriOnly.items[0].assets[0].mirrors[0].includes('/aweme/v1/play/?video_id=')) {
    throw new Error('playwm derivation: uri-only mirror');
  }
  return 'playwm_derivation_ok';
}

function selftestMusicFallback() {
  const MP3 = 'https://sf6-cdn-tos.douyinstatic.com/obj/ies-music/7342420221963029258.mp3';

  // Page-only shape: `music` has no play address; the BGM is wrapped as the
  // post's pseudo `video`.
  const pageShape = {
    aweme_id: 'gallery-3',
    aweme_type: 2,
    images: [{ uri: 'x', url_list: ['https://cdn.example/a.webp'], width: 1080, height: 1920 }],
    music: { mid: '1', title: 't', duration: 13, status: 1 },
    video: {
      play_addr: {
        uri: MP3,
        url_list: [`https://aweme.snssdk.com/aweme/v1/playwm/?video_id=${MP3}&ratio=720p&line=0`],
      },
    },
  };
  const pagePreview = buildPreview(selectMedia(pageShape));
  if (!pagePreview.music) throw new Error('music fallback: page shape lost the BGM');
  if (pagePreview.music.mirrors[0] !== MP3) {
    throw new Error('music fallback: mirror ' + pagePreview.music.mirrors[0]);
  }
  // ... and the music must be listed even though no live clip was found.
  if (pagePreview.liveItemCount !== 0) throw new Error('music fallback: unexpected live item');

  // A real music.play_url still wins over the pseudo-video fallback.
  const withPlayUrl = {
    aweme_id: 'gallery-4',
    images: [{ url_list: ['https://cdn.example/a.webp'] }],
    music: { play_url: { uri: 'https://sf1.example/obj/ies-music/real.mp3', url_list: ['https://sf1.example/obj/ies-music/real.mp3'] } },
    video: { play_addr: { uri: MP3 } },
  };
  const got = collectMusicMirrors(withPlayUrl);
  if (got.length !== 1 || got[0] !== 'https://sf1.example/obj/ies-music/real.mp3') {
    throw new Error('music fallback: play_url not preferred: ' + JSON.stringify(got));
  }

  // A video post (uri is a video id, not an mp3) must not gain an audio mirror.
  const videoShape = {
    aweme_id: 'video-12',
    video: { play_addr: { uri: 'v0d00fg10000dakq09vog65o77e0k78g', url_list: ['https://cdn.example/v.mp4'] } },
  };
  if (collectMusicMirrors(videoShape).length !== 0) {
    throw new Error('music fallback: video post gained audio');
  }
  return 'music_fallback_ok';
}
...
function handleSelfTest() {
  ...
  try {
    ...
    checks.push(selftestMusicFallback());
    checks.push(selftestExtractors());
    checks.push(selftestPlaywmDerivation());
